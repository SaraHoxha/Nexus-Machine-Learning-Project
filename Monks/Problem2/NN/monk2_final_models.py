...
for i in range(num_models):
    print (f'Model_{i}')
    model = models[i]
    history = model.fit(inputs_training, targets_training, epochs=250, validation_split=0.2, validation_data=(inputs_test,targets_test),workers=-1,verbose=1,shuffle=True)
    ...
    np.savetxt(f"monk+s+problems/Problem2/predictedlabels/training_pred_labels_model_{i}.csv", training_pred_labels, delimiter=",")
    test_pred_labels = model.predict(inputs_test, batch_size=None, verbose="auto", steps=None, callbacks=None)
    # Save the array as a CSV file
    np.savetxt(f"monk+s+problems/Problem2/predictedlabels/test_pred_labels_model_{i}.csv", test_pred_labels, delimiter=",")
    ...
    plt.legend()
    plt.title('Training curves for loss Monk2')
    plt.savefig(f'monk+s+problems/Problem2/plots/loss_model_{i}.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.title('Training curves for accuracy Monk2')
    plt.savefig(f'monk+s+problems/Problem2/plots/acc_model_{i}.png')
    #plt.show()
    plt.clf()