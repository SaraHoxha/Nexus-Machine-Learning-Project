...
print(f"For Random Forest in regards to Monk 2 Training, best parameters are {rf_search.best_params_} with a score of {rf_search.best_score_:0.5f}")
...
print(f"For Random Forest in regards to Monk 2 Testing, accuracy score is {test_accuracy:0.5f}")
mse = mean_squared_error(Y_test, predictions_test)
print(f"For Random Forest in regards to Monk 2 Testing, MSE is {mse:0.5f}")

cmTest = confusion_matrix(Y_test, predictions_test)
disp = ConfusionMatrixDisplay(confusion_matrix=cmTest)
disp.plot()
plt.title('Confusion Matrix Test Monk 2 Random Forest')
