...
#Reconstructing each of the best methods
    reconstructed_model = keras.models.load_model(f'Monks/Problem3/NN/Models/keras/model_{i}.keras')
...
for i in range(num_models):
    ...
    np.savetxt(f"Monks/Problem3/NN/predictedlabels/training_pred_labels_model_{i}.csv", training_pred_labels, delimiter=",")
    test_pred_labels = model.predict(inputs_test, batch_size=None, verbose="auto", steps=None, callbacks=None)
    # Save the array as a CSV file
    np.savetxt(f"Monks/Problem3/NN/predictedlabels/test_pred_labels_model_{i}.csv", test_pred_labels, delimiter=",")
    ...
    plt.savefig(f'Monks/Problem3/NN/plots/loss_model_{i}.png')
    #plt.show()
    plt.clf()
    ...
    plt.savefig(f'Monks/Problem3/NN/plots/acc_model_{i}.png')
    #plt.show()
    plt.clf()