...
print(f"For KNN in regards to Monk 1 Training, best parameters are {knn_search.best_params_} with a score of {knn_search.best_score_:0.5f}")
...
print(f"For KNN in regards to Monk 1 Testing, accuracy score is {test_accuracy:0.5f}")
mse = mean_squared_error(Y_test, predictions_test)
print(f"For KNN in regards to Monk 1 Testing, MSE is {mse:0.5f}")

cmTest = confusion_matrix(Y_test, predictions_test)
disp = ConfusionMatrixDisplay(confusion_matrix=cmTest)
disp.plot()
plt.title('Confusion Matrix Test Monk 1 KNN')
