...
def learningCurveForDifferentDatasetSize(dataset_sizes, mee_values, validationAcc):
    ...
    plt.title('KNR Regression Learning Curve for Mean Euclidian Error')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Dataset Size')
    plt.ylabel('Accuracy')
    plt.title('KNR Learning Curve for Validation Accuracy')
    plt.legend()
    plt.grid(True)
    plt.xticks(dataset_sizes)  # Set x-axis ticks to dataset sizes
...
def plot3D(predictions, ground_truth, phase):
    ...
    # Set title
    ax.set_title('KNR 3D Scatter Plot ' + str(phase))

    # Add legend
    ax.legend()

    plt.savefig(path.join(path.dirname(__file__), "plots", "knr3DPlot.png"))