...
from knrUtils import  getTrainDatasetPath
...
for trainIndex, testIndex in kf.split(X):
    ...
    grid_search = GridSearchCV(KNeighborsRegressor(n_jobs=-1), param_grid, cv=kf, scoring='neg_mean_squared_error', return_train_score=False)
...
if best_model_info['model'] is None or avg_metrics['meanSquaredError'] < best_model_info['metrics']['meanSquaredError']:
    ...
    best_model_info['weight'] = best_model.weights
    best_model_info['n_neighbors'] = best_model.n_neighbors
    best_model_info['algorithm'] = best_model.algorithm


print(f"Best Model Information:\n"
      f"  - Weight: {best_model_info['weight']}\n"
      f"  - Neigbors: {best_model_info['n_neighbors']}\n"
      f"  - Algorithm: {best_model_info['algorithm']}\n"
      f"  - Metrics: {best_model_info['metrics']}")

#Retrain with full training set
knrBestModel = KNeighborsRegressor(weights=best_model_info['weight'], n_neighbors=best_model_info['n_neighbors'], algorithm=best_model_info['algorithm']).fit(X,Y)

file_path = path.join(path.abspath(path.dirname(__file__)), "results/knrBestModel.z")
joblib.dump(knrBestModel, file_path)