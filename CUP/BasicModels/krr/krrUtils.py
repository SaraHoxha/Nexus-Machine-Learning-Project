...
def learningCurveForDifferentDatasetSize(dataset_sizes, mee_values, validationAcc):
    ...
    plt.title('KRR Poly Regression Learning Curve for Mean Euclidian Error')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Dataset Size')
    plt.ylabel('Accuracy')
    plt.title('KRR Poly Regression Learning Curve for Validation Accuracy')
    plt.legend()
    plt.grid(True)
    plt.xticks(dataset_sizes)  # Set x-axis ticks to dataset sizes
...
def plot3D(predictions, ground_truth, phase):
    ...
    # Set title
    ax.set_title('KRR 3D Scatter Plot ' + str(phase))

    # Add legend
    ax.legend()

    plt.savefig(path.join(path.dirname(__file__), "plots", "krr3DPlot.png"))
    #plt.show()
    #plt.clf()
