...
for trainIndex, testIndex in kf.split(X):
    ...
    grid_search = GridSearchCV(Ridge(), param_grid, cv=kf, scoring='neg_mean_squared_error', return_train_score=False)
...
ridgeBestModel = Ridge(alpha=best_model_info['alpha']).fit(X,Y)

file_path = path.join(path.abspath(path.dirname(__file__)), "results/ridgeBestModel.z")
joblib.dump(ridgeBestModel, file_path)