...
for i in range(num_models):
    ...
    np.savetxt(f"NN/FinalNNselection/Results/Predicted_vals_best_models/training/training_pred_labels_model_{i}.csv", training_pred_labels, delimiter=",")
    test_pred_labels = model.predict(inputs_test, batch_size=None, verbose="auto", steps=None, callbacks=None)
    # Save the array as a CSV file
    np.savetxt(f"NN/FinalNNselection/Results/Predicted_vals_best_models/test/test_pred_labels_model_{i}.csv", test_pred_labels, delimiter=",")
    ...
    plt.savefig(f'NN/FinalNNselection/Results/Plots/Loss/complex_loss_model_{i}.png')
    #plt.show()
    plt.clf()
    ...
    plt.savefig(f'NN/FinalNNselection/Results/Plots/Acc/complex_acc_model_{i}.png')
    #plt.show()
    plt.clf()