...
for i in range(num_models):
    print (f'Model_{i}')
    model = models[i]
    history = model.fit(inputs_training, targets_training, batch_size=64, epochs=50, validation_split=0.2, validation_data=(inputs_test,targets_test),workers=-1,verbose=1,shuffle=True)
